...
create table `digital_resource` (
    ...
    `code` char(8) unique not null comment '资源码',
...
-- 触发器，若 team_name 为 null，则生成一个默认的团队名，如果没有触发器，那 team_name 不能为 null
delimiter //
...
create table `day_record` (
    ...
    `credit1` double not null default 0.0 comment '信心指数平均值',

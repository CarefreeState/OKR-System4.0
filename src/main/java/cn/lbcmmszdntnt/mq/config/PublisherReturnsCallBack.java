...
public class PublisherReturnsCallBack implements RabbitTemplate.ReturnsCallback {
    ...
    public void returnedMessage(ReturnedMessage returnedMessage) {
        ...
        Integer delay = returnedMessage.getMessage().getMessageProperties().getReceivedDelay(); // 不是 getDelay 哦
        // 如果不是延时交换机，却设置了 delay 大于 0，是不会延时的，所以是其他原因导致的（以防万一把消息记录到日志里）
        if(Objects.nonNull(delay) && delay.compareTo(0) > 0) {
            log.info("交换机 {}, 路由键 {} 消息 {} 延迟 {} s", returnedMessage.getExchange(), returnedMessage.getRoutingKey(), returnedMessage.getMessage().getMessageProperties(), TimeUnit.MILLISECONDS.toSeconds(delay));
            return;
        }
